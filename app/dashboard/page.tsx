...
const icons = {
  ...
  macro: (
    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      <path d="M12 2v6M12 2c3 0 5 2.5 5 6s-2 6-5 6-5-2.5-5-6 2-6 5-6Z" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M7 22c0-3.5 2.5-6 5-6s5 2.5 5 6" strokeLinecap="round" />
    </svg>
  ),
};
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px', maxWidth: '1100px', marginBottom: '24px' }}>
        <div style={panelStyle}>
          <div style={panelHeaderStyle}>
            ...
            <span style={eyebrowStyle}>5 open</span>
          </div>
          <Row icon={icons.workout} label="Workout" title="Upper body strength" sub="4 exercises · ~40 minutes" />
          <Row icon={icons.goal} label="Goal" title="Lose 1% body fat this month" sub="On track · 0.4% so far" />
          <Row icon={icons.habit} label="Habit" title="Drink 100oz of water" sub="Logged 60oz today" />
          <Row icon={icons.course} label="Course" title="Watch Module 3: Mindset Reset" sub="12 minutes · not started" />
          <Row icon={icons.call} label="Call" title="Monthly check-in call" sub="Friday at 2:00 PM" isLast />
        </div>

        <div style={panelStyle}>
          <div style={panelHeaderStyle}>
            <h2 style={panelTitleStyle}>Community</h2>
            <span style={eyebrowStyle}>6 new</span>
          </div>
          <Row icon={icons.mention} label="Mentions" title="2 mentions" sub="You were tagged in the #wins channel" />
          <Row icon={icons.pod} label="Pod" title="3 messages in your pod" sub="Accountability Pod 4" />
          <Row icon={icons.kudos} label="Reactions" title="1 new reaction" sub="On your check-in post" isLast />
        </div>
      </div>
        ...
        <div style={panelStyle}>
          <div style={panelHeaderStyle}>
            <div>
              ...
              <p style={{ ...rowSubStyle, marginTop: '4px' }}>Synced from Trainerize once that&apos;s connected.</p>
            </div>
